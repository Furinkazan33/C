...
config_var *cfg_var_new(char *name, char *value) {
	MALLOC(config_var, res,);
	MALLOCS(res->name, sizeof(char) * (strlen(name) + 1), free(res););
	MALLOCS(res->value, sizeof(char) * (strlen(value) + 1), free(res); free(res->name););
...
config_var *cfg_var_new_from_string(char *s) {
	...
	MALLOCS(res->name, sizeof(char) * (equal - s + 1), free(res););
	strncpy(res->name, s, equal - s);
	res->name[equal - s] = '\0';

	MALLOCS(res->value, sizeof(char) * (strlen(s) - (equal - s) + 1), free(res->name); free(res););
...
char *cfg_readline(FILE *f, int *n_read) {
	...
	MALLOCS(res, sizeof(char) * (alloc + 1), );
