...
ppm *ppm_read(ppm *dest) {
	...
	IF_LOGE(n != 2, "error reading magic", "", fclose(fd); return NULL);

	n = read_until_space(&width[0], fd); //width
	IF_LOGE(n <= 0, "error reading width", "", fclose(fd); return NULL);

	n = read_until_space(&height[0], fd); //height
	IF_LOGE(n <= 0, "error reading height", "", fclose(fd); return NULL);

	n = read_until_space(&depth[0], fd); //Max color value
	IF_LOGE(n <= 0, "error reading colors depth", "", fclose(fd); return NULL);
