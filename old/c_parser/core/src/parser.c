...
c_variable *parser_read_var_decl(list **p0, lex_type type) {
	...
	if(!res) {
		fprintf(stderr, "parser_read_var_decl : call to parser_new_define_int returned NULL\n");
		return NULL;
	}
...
c_variable *parser_read_var_decl_ptr(list **p0, lex_type type) {
	...
	if(!res) {
		fprintf(stderr, "parser_read_var_decl_ptr : call to parser_new_define_int returned NULL\n");
		return NULL;
	}
...
void c_variable_print(c_variable *c, FILE *file) {
...
#ifdef DEBUG
		parser_print_statement(s, stdout);
#endif
...
int parser_print_statement(void *stat, void *file) {
	c_statement *s = stat;
	c_stat_type type = s->type;

	fprintf(file, "parser_print_statement : %s : ", parser_CST_literal(type));
	...
	switch(type) {
		case CST_NONE:
			...
			break;
		case CST_COMMENT:
			c_comment_print(s->data, file);
			break;
		case CST_INCLUDE:
			c_include_print(s->data, file);
			break;
		case CST_DEFINE:
			c_variable_print(s->data, file);
			break;
		case CST_VAR_DECL:
		case CST_VAR_ASSIGN:
		case CST_VAR_DECL_ASSIGN:
			...
			break;
		default:
			fprintf((FILE *)file, "unknown type : [%d]\n", type);
			return 0;
	}
	return 1;
}
